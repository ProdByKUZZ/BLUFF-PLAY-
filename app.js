...
if (window.Telegram && Telegram.WebApp) {
    ...
    Telegram.WebApp.onEvent('viewportChanged', (e) => {
        ...
        resizeCanvas(); // Перерисовываем canvas при изменении размера
    });
...
let currentScoreElement, highScoreElement;
...
function resizeCanvas() {
    ...
    // Перерисовываем игру, если она не активна
    if (!gameRunning) {
...
textures.startScreen.src = 'https://i.ibb.co/3m4WTGxf/flappy-start.jpg';
...
function startGame() {
    ...
    bird.y = canvas.height / 2; // Стартовая позиция по центру
    bird.velocity = 0;
    bird.rotation = 0;
    pipes.length = 0;
    score = 0;
...
function setupTouchControls() {
    ...
    canvas.addEventListener('touchend', (e) => {
        ...
        if (touchStartY - touchEndY > 10) { // Свайп вверх
            jump();
        }
    }, { passive: false });
}
...
function init() {
    ...
    gameSection.innerHTML = `
        <div class="score-container">
            <div class="score-box">
                Текущий: <span id="currentScore">0</span>
            </div>
            <div class="score-box">
        ...
        <div class="game-controls">
            <button class="game-btn" id="startButton">Старт</button>
            <button class="game-btn" id="backButton" style="display: none;">Назад</button>
        </div>
    `;
